...
class Subsampling(AmplificationBound):
    """Implement the bound from Privacy amplification by subsampling:
    Tight Analyses via Couplings and Divergences"""

    def __init__(self, name='subsampling', tol=None):
        super(Subsampling, self).__init__(name=name, tol=tol)


    def get_eps(self, eps0, n, m):
        """Implement the bound eps(eps0,n,m)"""
        eps = np.log(1+ (1 - (1-1/n)**m)* (np.exp(eps0) - 1))
        return eps

class Aggregation(AmplificationBound):
    """Implement the bound from Privacy amplification by subsampling:
    Tight Analyses via Couplings and Divergences"""

    def __init__(self, name='aggregat', tol=None):
        super(Aggregation, self).__init__(name=name, tol=tol)


    def get_eps(self, eps0, m):
        """Implement the bound eps(eps0,n,m)"""
        eps = eps0 / np.sqrt(m)
        return eps

class Erlingsson(AmplificationBound):
...
class NumericAmplificationBound(AmplificationBound):
    ...
    def __init__(self, mechanism, name, tol=None):
        ...
        super(NumericAmplificationBound, self).__init__(name=name, tol=tol)
...
class Hoeffding(NumericAmplificationBound):
...
class BennettExact(NumericAmplificationBound):
